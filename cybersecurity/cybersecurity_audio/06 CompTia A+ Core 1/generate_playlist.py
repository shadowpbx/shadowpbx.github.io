#!/usr/bin/env python3
"""
CompTIA A+ Core 2 (220-1202) Playlist Generator CLI Tool
...
def generate_playlist(dir, output, base_url):
    ...
    if not mp3_files:
        ...
        logger.info(f"Scanning local directory recursively: {os.path.abspath(local_dir)}")
        try:
            for root, dirs, files in os.walk(local_dir):
                ...
                for f in files:
                    if f.lower().endswith('.mp3'):
                        ...
                        mp3_files.append(normalized_path)
        except Exception as e:
