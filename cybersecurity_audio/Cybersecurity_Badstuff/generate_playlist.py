...
@click.option(
    '--dir', '-d', 
    default=None, 
    help='Local directory path OR remote HTTP/HTTPS URL containing audio files (defaults to "./Audio" if present, else ".").', 
    type=str
)
@click.option(
    '--output', '-o', 
...
@click.option(
    '--base-url', '-b',
    default='',
    help='Override default Cloudflare R2 base URL to prepend to audio tracks.'
)
def generate_playlist(dir, output, base_url):
    ...
    """
    # Auto-detect target scanning directory if not explicitly provided
    if not dir:
        if os.path.exists('./Audio') and os.path.isdir('./Audio'):
            scan_dir = './Audio'
        else:
            scan_dir = '.'
    else:
    ...
    is_remote = scan_dir.startswith('http://') or scan_dir.startswith('https://')
    mp3_files = []
    ...
    if is_remote:
        logger.info(f"Scanning remote URL: {scan_dir}")
        if not resolved_base_url:
            resolved_base_url = scan_dir
        ...
        try:
            ...
            if not mp3_files:
                logger.warning(
                    f"No MP3 files found at remote URL: {scan_dir} (Directory listing may be disabled).\n"
    ...
    if not mp3_files:
        # Fallback to scan local directory (prefer ./Audio if it exists)
        if os.path.exists('./Audio') and os.path.isdir('./Audio'):
            local_dir = './Audio'
        else:
            local_dir = '.' if is_remote else scan_dir
    ...
    # Resolve output names (always write to current directory or output folder specified)
    output_dir = os.path.dirname(output) if os.path.dirname(output) else '.'
    filename, ext = os.path.splitext(os.path.basename(output))
    base_name = filename if ext.lower() in ['.json', '.js'] else output
    ...
    tracks_list = []
    effective_local_dir = local_dir if 'local_dir' in locals() else scan_dir
    for f_name in mp3_files:
        local_file_path = f_name
        if not is_remote and effective_local_dir != '.':
            local_file_path = os.path.join(effective_local_dir, f_name)
        elif os.path.exists(os.path.join('./Audio', f_name)):
            local_file_path = os.path.join('./Audio', f_name)
    ...
    try:
        ...
        # Auto-compile concepts.md -> concepts.js if present (Single Source of Truth)
        concepts_md_path = os.path.join(output_dir, "concepts.md")
        concepts_js_path = os.path.join(output_dir, "concepts.js")
        if os.path.exists(concepts_md_path):
            try:
                with open(concepts_md_path, 'r', encoding='utf-8') as f:
                    raw_md = f.read()
                concepts_json = json.dumps(raw_md)
                with open(concepts_js_path, 'w', encoding='utf-8') as f:
                    f.write(f"// Auto-generated by generate_playlist.py. Do not edit manually.\nconst conceptsMarkdown = {concepts_json};\n")
                logger.info(f"Successfully compiled concepts.md into: {concepts_js_path}")
            except Exception as e:
                logger.warning(f"Could not compile concepts.js: {e}")
