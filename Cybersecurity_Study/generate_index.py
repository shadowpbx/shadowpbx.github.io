...
# Sub-pages
CERTS_FILE = 'certifications.html'
CYBER_FILE = 'cybersecurity_coursework.html'
ALL_FILE = 'all_coursework.html'
...
def convert_md_to_html(md_file):
    ...
    # Track classification (Certification or Academic)
    track = front_matter.get('track', 'Academic')
    track = str(track).strip().capitalize()
    if track not in ['Certification', 'Academic']:
        track = 'Academic'

    # Category classification (Cybersecurity or General/Gen-Ed)
    ...
    if not tag:
        ...
        if "a+" in title_lower or "aplus" in title_lower:
            tag = "A+"
        elif "security" in title_lower or "sec+" in title_lower:
            tag = "SEC+"
        elif "government" in title_lower or "gov" in title_lower:
            tag = "GOVERNMENT"
        elif "english" in title_lower or "writing" in title_lower:
            tag = "ENGLISH"
        elif "algebra" in title_lower or "math" in title_lower:
            tag = "ALGEBRA"
        else:
            tag = "STUDY"
...
def update_dashboard_index(certs_count, cyber_count, all_count):
    """
    ...
    # Update Certs Count
    content = re.sub(r'<!-- CERTS_COUNT -->.*?<!-- CERTS_COUNT_END -->', f'<!-- CERTS_COUNT -->{certs_count}<!-- CERTS_COUNT_END -->', content)
    
    # Update Cyber Coursework Count
    content = re.sub(r'<!-- CYBER_COUNT -->.*?<!-- CYBER_COUNT_END -->', f'<!-- CYBER_COUNT -->{cyber_count}<!-- CYBER_COUNT_END -->', content)
    
    # Update All Coursework Count
    content = re.sub(r'<!-- ALL_COUNT -->.*?<!-- ALL_COUNT_END -->', f'<!-- ALL_COUNT -->{all_count}<!-- ALL_COUNT_END -->', content)
...
def main():
    ...
    # Group posts
    certs_posts = [p for p in all_posts_metadata if p['track'] == 'Certification']
    
    # Cybersecurity Coursework = Academic Track & Category is Cybersecurity
    cyber_posts = [p for p in all_posts_metadata if p['track'] == 'Academic' and p['category'] == 'Cybersecurity']
    
    # All Coursework = All Academic Track posts (both general Gen-Ed and Cybersecurity Coursework!)
    all_coursework_posts = [p for p in all_posts_metadata if p['track'] == 'Academic']

    # Update list sub-pages
    update_page_list(CERTS_FILE, certs_posts)
    update_page_list(CYBER_FILE, cyber_posts)
    update_page_list(ALL_FILE, all_coursework_posts)

    # Update dashboard file counters
    update_dashboard_index(len(certs_posts), len(cyber_posts), len(all_coursework_posts))
